...
interface CreateTestTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: { name: string; results: string[] }) => void;
}

const CreateTestTypeModal: React.FC<CreateTestTypeModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
}) => {
  ...
  const handleSubmit = () => {
    ...
    if (testTypeName.trim() && filteredResults.length > 0) {
      onSubmit({ name: testTypeName, results: filteredResults });
      // Reset form
      setTestTypeName('');
      setResults(['', '']);
      onClose();
