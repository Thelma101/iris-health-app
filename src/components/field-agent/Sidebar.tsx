...
export default function FieldAgentSidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-[267px] bg-[#ecf4ff] z-40 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          }`}
      >
        <div className="flex flex-col h-full pt-[72px] px-[35px] pb-8">
          {/* Navigation */}
          <nav className="flex flex-col gap-3.5 w-[200px]">
            {menuItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={item.path}
                  onClick={onClose}
                  className={`flex items-center gap-2.5 p-1.5 rounded-lg transition-all cursor-pointer w-full ${isActive
            ...
            <button
              ...
              className="flex items-center gap-2.5 p-1.5 rounded-lg transition-all cursor-pointer w-full text-[#d64545] hover:bg-red-50"
            >
