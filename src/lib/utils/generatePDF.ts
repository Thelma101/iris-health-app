...
export function generatePatientReportPDF(patient: PatientPDFData): jsPDF {
  ...
  if (sortedTests.length === 0) {
    ...
    sortedTests.forEach((test, idx) => {
      ...
      if (test.officerNote) {
        const noteLines = doc.splitTextToSize(`Additional Notes: ${test.officerNote}`, contentWidth - 10);
  ...
  if (patient.adminEmail) {
    infoItems.push(['GENERATED BY', patient.adminEmail]);
  }
    ...
    if (p.testDetails?.length) {
      ...
      sorted.forEach((t: any, ti: number) => {
        ...
        if (t.officerNotes) {
          doc.setFontSize(7);
          doc.setTextColor(130, 130, 130);
          const noteLines = doc.splitTextToSize(`    Additional Notes: ${t.officerNotes}`, contentWidth - 15);
          doc.text(noteLines, margin + 5, y);
