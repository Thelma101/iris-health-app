...
export default function FieldAgentLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://lunar-med-track-backend.onrender.com/api'}${endpoint}`, {
        method: 'POST',
      ...
      if (!res.ok) {
        // Provide specific error messages based on login type and error
        const errorMessage = data.message || 'Login failed';
        if (errorMessage.toLowerCase().includes('not found') || errorMessage.toLowerCase().includes('does not exist') || errorMessage.toLowerCase().includes('invalid')) {
          if (loginType === 'field-agent') {
            throw new Error('Field Agent credentials not found. Please check your email and password, or switch to Admin login.');
          } else {
            throw new Error('Admin credentials not found. Please check your email and password, or switch to Field Agent login.');
          }
        }
  ...
  return (
    ...
    >
      <div className="w-full max-w-[400px] flex flex-col items-center gap-8 bg-white/95 backdrop-blur-sm rounded-2xl p-8 shadow-lg">
        ...
        {/* Login Type Tabs */}
        <div className="flex items-center gap-1 bg-[#f4f6f8] p-1 rounded-[10px]">
          <button
            type="button"
            onClick={() => setLoginType('field-agent')}
            className={`h-10 px-6 rounded-[8px] font-poppins text-sm font-medium transition-colors ${loginType === 'field-agent'
          ...
          <button
            ...
            className={`h-10 px-6 rounded-[8px] font-poppins text-sm font-medium transition-colors ${loginType === 'admin'
                ? 'bg-[#2c7be5] text-white shadow-sm'
                : 'text-[#637381] hover:text-[#212b36]'
              }`}
          >
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                placeholder="Email"
                className="h-12 px-4 bg-white border border-[#d9d9d9] rounded-lg font-poppins text-sm text-[#212b36] placeholder:text-[#919eab] focus:outline-none focus:border-[#2c7be5] focus:ring-1 focus:ring-[#2c7be5]"
                required
              />
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••••••••••"
                  className="w-full h-12 px-4 pr-12 bg-white border border-[#d9d9d9] rounded-lg font-poppins text-sm text-[#212b36] placeholder:text-[#919eab] focus:outline-none focus:border-[#2c7be5] focus:ring-1 focus:ring-[#2c7be5]"
                  required
                />
                <button
                  type="button"
              ...
              <div className="flex justify-end mt-1">
                <Link
                  href="/forgot"
                  className="font-poppins text-sm text-[#2c7be5] hover:text-[#1e5aa8] hover:underline"
                >
                  Forgot Password
                </Link>
              </div>
          ...
          {/* Error Message */}
          {error && (
            <div className="p-3 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm font-poppins">
              {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="h-12 bg-[#2c7be5] text-white rounded-lg font-poppins font-semibold text-base hover:bg-[#1e5aa8] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
