...
export const getAdminById = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  const admin = await adminModel.findById(id).select('-password');
  if (!admin) {
    throw new NotFoundException('Admin not found', ERRORCODES.RESOURCE_NOT_FOUND);
  }
  new SuccessResponse('Admin fetched successfully.', { admin }).sendResponse(res);
});

export const updateAdmin = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
  ...
  if (status) (admin as any).status = status;
